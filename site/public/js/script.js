...
$(function(){
    ...
    $("main .district").change(function(event) { // change thay đổi tỉnh
        ...
        $.ajax({
            url: 'index.php?c=cart&a=display', // yêu cầu chạy hàm display 
            type: 'GET'
        })
...
function displayCart(data) {
    var cart = JSON.parse(data); // hàm JSON.parse chuyển chuỗi có dạng json sang array hoặc oject
...
function updateSelectBox(data, selector) {
    ...
    $(selector).find('option').not(':first').remove(); // tìm option loại trừ đi cái đầu tiên(quận/huyện, phường/xã) còn lại xóa đi
    if (!data) return;
